...
n_name <- function(x, nm) sum(x$analysis$analysis_eligible & x$analysis$name_expanded == nm, na.rm = TRUE)
measure <- function(x) c(source_rows = nrow(x$records), counted_source_rows = sum(x$records$counted),
  ...
  museum_of_illusions_name = n_name(x, "museum of illusions"),
  washington_county_name = n_name(x, "washington county historical society"),
  old_jail_name = n_name(x, "old jail museum"),
  verified_institutions = sum(x$analysis$review_status == "verified"))
expected_before <- c(60002L, 57305L, 52597L, 52465L, 13L, 13L, 12L, 3L)
expected_after <- c(60002L, 57293L, 52588L, 52456L, 12L, 7L, 12L, 16L)
...
check("Records outside 19 reviewed members unchanged", identical(before$records[!affected, ], after$records[!affected, ]))
check("Eight identity cases with 19 members", nrow(added) == 19L && dplyr::n_distinct(added$case_id) == 8L)
check("Every affected baseline member explicitly listed", setequal(
  paste(baseline$source[baseline$entity_id %in% added$expected_entity_id], baseline$source_id[baseline$entity_id %in% added$expected_entity_id]),
  paste(added$source, added$source_id)))
accepted <- added[added$role != "source_conflict", ]
check("One counted representative per accepted case", all(vapply(split(accepted$source_id, accepted$case_id),
  function(ids) sum(after$records$counted[after$records$source_id %in% ids]) == 1L, logical(1))))
ar <- after$records[after$records$source_id == "8400500101", ]
check("Arkansas association row isolated and uncounted", nrow(ar) == 1L && !ar$counted && ar$exclusion_reason == "reviewed_source_conflict")
check("Five source conflicts isolated", sum(after$records$exclusion_reason == "reviewed_source_conflict", na.rm = TRUE) == 5L)
...
check("Publication gate rejects Museum of Illusions while Miami is pending", !gate("museum of illusions"))
check("Publication gate rejects Washington County Historical Society", !gate("washington county historical society"))
moi <- after$analysis[after$analysis$analysis_eligible & after$analysis$name_expanded == "museum of illusions", ]
check("Eleven verified network locations plus Miami pending",
  sum(moi$review_status == "verified" & moi$affiliation_status == "chain") == 11L && sum(moi$review_status == "pending") == 1L)
was_verified <- before$analysis$entity_id[before$analysis$review_status == "verified"]
check("Thirteen new complete factual reviews",
  sum(after$analysis$review_status == "verified" & !after$analysis$entity_id %in% was_verified) == 13L)
...
check("90 identity rows in 36 cases", nrow(live_ids) == 90L && dplyr::n_distinct(live_ids$case_id) == 36L)
check("Matching threshold remains 0.85", identical(DN_NAME_SIM_MIN, 0.85))
if (file.exists("data/processed/leaders_review_before.rds")) {
  saved_before <- readRDS("data/processed/leaders_review_before.rds")
